...
var
  ...
  Player: TPlayer;
  TokenDB, TokenReceived: string;
  SecondsDiff: Integer;
begin
  Result := False;

  // Converte para string normal (evita erros de comparação entre tipos)
  TokenReceived := string(Packet.Token);

  if Player.LoadAccountSQL(string(Packet.Username)) then
  begin
    TokenDB := string(Player.Account.Header.Token.Token);

    // DEBUG: loga os tokens recebidos e salvos
    Logger.Write('[CheckToken] Token do Banco: ' + TokenDB, TLogType.ConnectionsTraffic);
    Logger.Write('[CheckToken] Token Recebido: ' + TokenReceived, TLogType.ConnectionsTraffic);
    Logger.Write('[CheckToken] Token Criado em: ' + DateTimeToStr(Player.Account.Header.Token.CreationTime), TLogType.ConnectionsTraffic);

    // Verifica se o token bate e se está dentro dos 5 minutos de validade
    SecondsDiff := SecondsBetween(Now, Player.Account.Header.Token.CreationTime);
    Logger.Write('[CheckToken] Diferença em segundos: ' + SecondsDiff.ToString, TLogType.ConnectionsTraffic);

    if (AnsiCompareText(TokenDB, TokenReceived) = 0) and (SecondsDiff < 300) then
    begin
      if Player.Account.Header.AccountStatus = 8 then
      begin
        Logger.Write('[CheckToken] Conta banida. Encerrando conexão.', TLogType.Warnings);
        Connection.Destroy;
        Exit;
      end;
      ...
      Logger.Write('[CheckToken] Login aceito para: ' + Packet.Username, TLogType.ConnectionsTraffic);
    ...
    begin
      Logger.Write('[CheckToken] Token inválido ou expirado para usuário: ' + Packet.Username, TLogType.ConnectionsTraffic);
      Connection.Destroy;
    end;
  end
  else
  begin
    Logger.Write('[CheckToken] Falha ao carregar conta do usuário: ' + Packet.Username, TLogType.Warnings);
    Connection.Destroy;
  end;
      ...
      10000, -10000:
        begin
          ...
          if(ProductReceived > 0) then
            Servers[ServerID].Players[ClientID].SendClientMessage('Suas [10.000 iNiz Coins] foram entregues, confira apertando J.', 16, 32, 16)
            else
